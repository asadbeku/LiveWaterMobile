...
class AddUserRepository {
    ...
    suspend fun addUser(json: JsonObject) = flow {
        ...
        val response = Network.buildService(ApiService::class.java).addUser(json)
    ...
    suspend fun changeUser(id: String, userJson: JsonObject) = flow {
        try {
            val response = Network.buildService(ApiService::class.java).updateUser(id, userJson)
    ...
    fun removeUser(id: String) = flow<Boolean> {
        val response = Network.buildService(ApiService::class.java).deleteUser(id)
