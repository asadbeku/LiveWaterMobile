package uz.prestige.livewater.regions.view_model
...
class RegionsRepository {
    ...
    suspend fun updateRegion(id: String, json: JsonObject) = flow {
        val response = Network.buildService(ApiService::class.java).updateRegion(id, json)
    ...
    suspend fun deleteRegion(id: String) = flow {
        val response = Network.buildService(ApiService::class.java).removeRegion(id = id)
    ...
    suspend fun addRegion(json: JsonObject) = flow {
        ...
        val response = Network.buildService(ApiService::class.java).addRegion(json)
